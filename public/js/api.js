    ...
    fetch(url)
        ...
        .then(data => {
            ...
            for(let i = 0; i < data.features.length; i++){
                ...
                // Creates the Div for the card
                newAlert = document.createElement('div')

                // Determines what color to make the card
                const severityData = alertData.properties.severity;
                if(severityData === 'Extreme') {
                    newAlert.className = 'card bg-purple presentationCard'
                } else if(severityData === "Severe") {
                    newAlert.className = 'card bg-red presentationCard'
                } else if(severityData === "Moderate") {
                    newAlert.className = 'card bg-orange presentationCard'
                } else if(severityData === 'Minor') {
                    newAlert.className = 'card bg-yellow presentationCard'
                } else {
                    newAlert.className = 'card bg-gray presentationCard'
                }

                // create the internal Div used for styling
                ...
                severity.innerHTML = '<strong>Severity: </strong>' + severityData
